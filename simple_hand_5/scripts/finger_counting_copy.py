...
# Initializing the desired velocities and positions
    ...
    t_hold = 4 # Time to hold fingers in correct position
    step_no = 1

    # inputting the thumb

    th_thumb_start = np.array([[1], [0], [1], [1]])
    p_thumb_start = np.zeros((3,1))
    R_thumb_start = np.identity(3)

    kin_thumb.fkin(th_thumb_start, p_thumb_start, R_thumb_start)

    (pd_thumb, vd_thumb) = desired(0, tf, p_thumb_start, p_thumb_start)
    ...
    while not rospy.is_shutdown():
        ...
        if desiredNum != desiredNumNew or handState != handStateNew:
            if handStateNew == h_open:
                kin_thumb.fkin(th_thumb_start, p_t0, R_t0)
            else:
        ...
        if handState == h_free:
            ...
            (p_thumb_goal, R_thumb_goal) = (p_thumb_start, R_thumb_start)
            (p_index_goal, R_index_goal) = (p_index_open, R_index_open)
            (p_middle_goal, R_middle_goal) = (p_middle_open, R_middle_open)
            (p_ring_goal, R_ring_goal) = (p_ring_open, R_ring_open)
            (p_pinky_goal, R_pinky_goal) = (p_pinky_open, R_pinky_open)
